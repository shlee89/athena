 ...
 */
public class AggregateStatsCollector implements TimerTask {
     ...
     */
    public AggregateStatsCollector(OpenFlowSwitch sw, int interval) {
        this.sw = sw;
    ...
    public synchronized void start() {
        log.info("Starting Aggregate Stats collection thread for {}", sw.getStringId());
        stopped = false;
    ...
    public synchronized void stop() {
        log.info("Stopping Aggregate Stats collection thread for {}", sw.getStringId());
        stopped = true;
        timeout.cancel();
    }
}
