 ...
 */
public class TableStatsCollector implements TimerTask {
     ...
     */
    public TableStatsCollector(OpenFlowSwitch sw, int interval) {
        this.sw = sw;
    ...
    public void run(Timeout to) throws Exception {
        ...
        log.trace("Collecting Table stats for {}", sw.getStringId());
    ...
    private void sendTableStatistic() {
        if (sw.getRole() != RoleState.MASTER) {
            return;
        }
        //Long statsXid = xidAtomic.getAndIncrement();
        if (sw.factory().getVersion().compareTo(OFVersion.OF_10) > 0) {
            OFTableStatsRequest tableRequest = sw.factory()
                    .buildTableStatsRequest()
                    .setXid(1130)
                    .build();
            sw.sendMsg(tableRequest);
        }
    }
