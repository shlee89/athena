...
public class FlowStatsCollector implements TimerTask {
     ...
     */
    public FlowStatsCollector(OpenFlowSwitch sw, int interval) {
        this.sw = sw;
    ...
    public synchronized void start() {
        log.info("Starting Flow Stats collection thread for {}", sw.getStringId());
        stopped = false;
    ...
    public synchronized void stop() {
        log.info("Stopping Flow Stats collection thread for {}", sw.getStringId());
        stopped = true;
        timeout.cancel();
    }
}
