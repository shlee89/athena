...
public class OpenFlowDeviceProvider extends AbstractProvider implements DeviceProvider {
    ...
    /**
     * This field is used for featureCollectors.
     * -Seunghyeon
     */
    private final InternalDeviceProvider listener = new InternalDeviceProvider();

    // TODO: We need to make the poll interval configurable.
    static final int PORT_POLL_INTERVAL = 5;
    static final int AGGREGATE_POLL_INTERVAL = 5;
    static final int FLOW_POLL_INTERVAL = 5;
    static final int QUEUE_POLL_INTERVAL = 5;
    static final int TABLE_POLL_INTERVAL = 5;

    @Property(name = "PortStatsPollFrequency", intValue = PORT_POLL_INTERVAL,
            label = "Frequency (in seconds) for polling switch Port statistics")
    private int portStatsPollFrequency = PORT_POLL_INTERVAL;

    @Property(name = "AggregateStatsPollFrequency", intValue = AGGREGATE_POLL_INTERVAL,
            label = "Frequency (in seconds) for polling switch Aggregate statistics")
    private int aggregateStatsPollFrequency = AGGREGATE_POLL_INTERVAL;

    @Property(name = "FlowStatsPollFrequency", intValue = FLOW_POLL_INTERVAL,
            label = "Frequency (in seconds) for polling switch Flow statistics")
    private int flowStatsPollFrequency = FLOW_POLL_INTERVAL;

    @Property(name = "QueueStatsPollFrequency", intValue = QUEUE_POLL_INTERVAL,
            label = "Frequency (in seconds) for polling switch Queue statistics")
    private int queueStatsPollFrequency = QUEUE_POLL_INTERVAL;

    @Property(name = "TableStatsPollFrequency", intValue = TABLE_POLL_INTERVAL,
            label = "Frequency (in seconds) for polling switch Table statistics")
    private int tableStatsPollFrequency = TABLE_POLL_INTERVAL;

    /**
     * Add additional statistics controllers.
     * -Seunghyeon
     */
    private HashMap<Dpid, PortStatsCollector> portCollectors = Maps.newHashMap();
    private HashMap<Dpid, TableStatsCollector> tableCollectors = Maps.newHashMap();
    private HashMap<Dpid, QueueStatsCollector> queueCollectors = Maps.newHashMap();
    private HashMap<Dpid, FlowStatsCollector> flowCollectors = Maps.newHashMap();
    private HashMap<Dpid, AggregateStatsCollector> aggregateCollectors = Maps.newHashMap();
    ...
    /**
     * Seunghyeon.
     * Add addtional statistics collectors
     *
     * @param context
     */
    @Modified
    public void modified(ComponentContext context) {
        ...
        int newAggregateStatsPollFrequency;
        int newFlowStatsPollFrequency;
        int newQueueStatsPollFrequency;
        int newTableStatsPollFrequency;
        try {
            ...
            s = get(properties, "AggregateStatsPollFrequency");
            newAggregateStatsPollFrequency =
                    isNullOrEmpty(s) ? aggregateStatsPollFrequency : Integer.parseInt(s.trim());

            s = get(properties, "FlowStatsPollFrequency");
            newFlowStatsPollFrequency =
                    isNullOrEmpty(s) ? flowStatsPollFrequency : Integer.parseInt(s.trim());

            s = get(properties, "QueueStatsPollFrequency");
            newQueueStatsPollFrequency =
                    isNullOrEmpty(s) ? queueStatsPollFrequency : Integer.parseInt(s.trim());

            s = get(properties, "TableStatsPollFrequency");
            newTableStatsPollFrequency =
                    isNullOrEmpty(s) ? tableStatsPollFrequency : Integer.parseInt(s.trim());

        } catch (NumberFormatException | ClassCastException e) {
            newPortStatsPollFrequency = portStatsPollFrequency;
            newAggregateStatsPollFrequency = aggregateStatsPollFrequency;
            newFlowStatsPollFrequency = flowStatsPollFrequency;
            newQueueStatsPollFrequency = queueStatsPollFrequency;
            newTableStatsPollFrequency = tableStatsPollFrequency;

        }

        if (newPortStatsPollFrequency != portStatsPollFrequency) {
            portStatsPollFrequency = newPortStatsPollFrequency;
            portCollectors.values().forEach(psc -> psc.adjustPollInterval(portStatsPollFrequency));

        }

        if (newAggregateStatsPollFrequency != aggregateStatsPollFrequency) {
            aggregateStatsPollFrequency = newAggregateStatsPollFrequency;
            aggregateCollectors.values().forEach(psc -> psc.adjustPollInterval(aggregateStatsPollFrequency));

        }

        if (newFlowStatsPollFrequency != flowStatsPollFrequency) {
            flowStatsPollFrequency = newFlowStatsPollFrequency;
            flowCollectors.values().forEach(psc -> psc.adjustPollInterval(flowStatsPollFrequency));

        }

        if (newQueueStatsPollFrequency != queueStatsPollFrequency) {
            queueStatsPollFrequency = newQueueStatsPollFrequency;
            queueCollectors.values().forEach(psc -> psc.adjustPollInterval(queueStatsPollFrequency));

        }

        if (newTableStatsPollFrequency != tableStatsPollFrequency) {
            tableStatsPollFrequency = newTableStatsPollFrequency;
            tableCollectors.values().forEach(psc -> psc.adjustPollInterval(tableStatsPollFrequency));

        }

        LOG.info("Settings: portStatsPollFrequency={}", portStatsPollFrequency);
        LOG.info("Settings: aggregateStatsPollFrequency={}", aggregateStatsPollFrequency);
        LOG.info("Settings: flowStatsPollFrequency={}", flowStatsPollFrequency);
        LOG.info("Settings: queueStatsPollFrequency={}", queueStatsPollFrequency);
        LOG.info("Settings: tableStatsPollFrequency={}", tableStatsPollFrequency);
    }
    ...
    private void connectInitialDevices() {
        for (OpenFlowSwitch sw : controller.getSwitches()) {
            ...
            /**
             * Add additional statistics requesters.
             * - Seunghyeon
             */
            PortStatsCollector psc =
                    new PortStatsCollector(sw, portStatsPollFrequency);
            if (!portCollectors.containsKey(new Dpid(sw.getId()))) {
                psc.start();
                portCollectors.putIfAbsent(new Dpid(sw.getId()), psc);
            }
            AggregateStatsCollector asc =
                    new AggregateStatsCollector(sw, aggregateStatsPollFrequency);
            if (!aggregateCollectors.containsKey(new Dpid(sw.getId()))) {
                asc.start();
                aggregateCollectors.putIfAbsent(new Dpid(sw.getId()), asc);
            }

            FlowStatsCollector fsc =
                    new FlowStatsCollector(sw, flowStatsPollFrequency);
            if (!flowCollectors.containsKey(new Dpid(sw.getId()))) {
                fsc.start();
                flowCollectors.putIfAbsent(new Dpid(sw.getId()), fsc);
            }

            QueueStatsCollector qsc =
                    new QueueStatsCollector(sw, queueStatsPollFrequency);
            if (!queueCollectors.containsKey(new Dpid(sw.getId()))) {
                qsc.start();
                queueCollectors.putIfAbsent(new Dpid(sw.getId()), qsc);
            }

            TableStatsCollector tsc =
                    new TableStatsCollector(sw, tableStatsPollFrequency);
            if (!tableCollectors.containsKey(new Dpid(sw.getId()))) {
                tsc.start();
                tableCollectors.putIfAbsent(new Dpid(sw.getId()), tsc);
            }
        }
    }
    ...
    private class InternalDeviceProvider implements OpenFlowSwitchListener, OpenFlowEventListener {
        ...
        public void switchRemoved(Dpid dpid) {
             ...
             * - Seunghyeon
             */
            PortStatsCollector localPortCollector = portCollectors.remove(dpid);
            if (localPortCollector != null) {
                localPortCollector.stop();
                LOG.debug("switchRemoved: localPortCollector.stop()");
            }

            AggregateStatsCollector localAggregateCollector = aggregateCollectors.remove(dpid);
            if (localAggregateCollector != null) {
                localAggregateCollector.stop();
                LOG.debug("switchRemoved: localAggregateCollector.stop()");
            }

            FlowStatsCollector localFlowCollector = flowCollectors.remove(dpid);
            if (localFlowCollector != null) {
                localFlowCollector.stop();
                LOG.debug("switchRemoved: localFlowCollector.stop()");
            }

            QueueStatsCollector localQueueCollector = queueCollectors.remove(dpid);
            if (localQueueCollector != null) {
                localQueueCollector.stop();
                LOG.debug("switchRemoved: localQueueCollector.stop()");
            }

            TableStatsCollector localTableCollector = tableCollectors.remove(dpid);
            if (localTableCollector != null) {
                localTableCollector.stop();
                LOG.debug("switchRemoved: localTableCollector.stop()");
            }
