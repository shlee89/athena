...
#sudo python athena.py $OC1 $OC2 $OC2
#onos-netcfg $OCI $ONOS_ROOT/tools/test/topos/athena-cfg.json
class AttMplsTopo( Topo ):
    ...
    def __init__( self ):
        ...
        NY54 = self.addSwitch( 's25', protocols='OpenFlow13' ) # 40.728270, -73.994483
        CMBR = self.addSwitch( 's1', protocols='OpenFlow13' )  # 42.373730, -71.109734
        CHCG = self.addSwitch( 's2', protocols='OpenFlow13' )  # 41.877461, -87.642892
        CLEV = self.addSwitch( 's3', protocols='OpenFlow13' )  # 41.498928, -81.695217
        RLGH = self.addSwitch( 's4', protocols='OpenFlow13' )  # 35.780150, -78.644026
        ATLN = self.addSwitch( 's5', protocols='OpenFlow13' )  # 33.749017, -84.394168
        PHLA = self.addSwitch( 's6', protocols='OpenFlow13' )  # 39.952906, -75.172278
        WASH = self.addSwitch( 's7', protocols='OpenFlow13' )  # 38.906696, -77.035509
        NSVL = self.addSwitch( 's8', protocols='OpenFlow13' )  # 36.166410, -86.787305
        STLS = self.addSwitch( 's9', protocols='OpenFlow13' )  # 38.626418, -90.198143
        NWOR = self.addSwitch( 's10', protocols='OpenFlow13'  ) # 29.951475, -90.078434
        HSTN = self.addSwitch( 's11', protocols='OpenFlow13'  ) # 29.763249, -95.368332
        SNAN = self.addSwitch( 's12', protocols='OpenFlow13'  ) # 29.424331, -98.491745
        DLLS = self.addSwitch( 's13', protocols='OpenFlow13'  ) # 32.777665, -96.802064
        ORLD = self.addSwitch( 's14', protocols='OpenFlow13'  ) # 28.538641, -81.381110
        DNVR = self.addSwitch( 's15', protocols='OpenFlow13'  ) # 39.736623, -104.984887
        KSCY = self.addSwitch( 's16', protocols='OpenFlow13'  ) # 39.100725, -94.581228
        SNFN = self.addSwitch( 's17', protocols='OpenFlow13'  ) # 37.779751, -122.409791
        SCRM = self.addSwitch( 's18', protocols='OpenFlow13'  ) # 38.581001, -121.497844
        PTLD = self.addSwitch( 's19', protocols='OpenFlow13'  ) # 45.523317, -122.677768
        STTL = self.addSwitch( 's20', protocols='OpenFlow13'  ) # 47.607326, -122.331786
        SLKC = self.addSwitch( 's21', protocols='OpenFlow13'  ) # 40.759577, -111.895079
        LA03 = self.addSwitch( 's22', protocols='OpenFlow13' ) # 34.056346, -118.235951
        SNDG = self.addSwitch( 's23', protocols='OpenFlow13'  ) # 32.714564, -117.153528
        PHNX = self.addSwitch( 's24', protocols='OpenFlow13'  ) # 33.448289, -112.076299
...
#        PHNX_host = self.addHost( 'h24' )
        ...
        self.addLink( NWOR , WASH_host )
        self.addLink( NWOR , NSVL_host )
        self.addLink( NWOR , STLS_host )
        self.addLink( NWOR , NWOR_host )
        self.addLink( ORLD , HSTN_host )
        self.addLink( ORLD , SNAN_host )
        self.addLink( ORLD , DLLS_host )
